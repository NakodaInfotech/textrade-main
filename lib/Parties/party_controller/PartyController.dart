...
class PartyController extends GetxController {
  ...
  var mainLedgerResponse = LedgerMainResponseModel().obs;
  ...
  void applyFilter() async {
    ...
    final String ledger = (filterLedgerResponse.value.ledger ?? [])
    ...
    final String agent = (filterAgentResponse.value.agent ?? [])
        ...
        .map((e) => e.value)
        .join(",");
    final String city = (filterCityResponse.value.city ?? [])
        .where((p0) => p0.isSelected == true)
        .map((e) => e.value)
        .join(",");
    final String area = (filterAreaResponse.value.aREA ?? [])
        .where((p0) => p0.isSelected == true)
        .map((e) => e.value)
        .join(",");
    final String group = (filterGroupResponse.value.group ?? [])
        .where((p0) => p0.isSelected == true)
        .map((e) => e.value)
        .join(",");
