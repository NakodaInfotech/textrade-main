...
class OutStandingFilterScreen extends StatelessWidget {
  var controller = Get.find<OutstandingController>();
  OutStandingFilterScreen({super.key});
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            Expanded(
              flex: 4,
              child: GetX<OutstandingController>(builder: (controller) {
                return Container(
                  child: Column(
                    children: [
                      searchBar(),
  ...
  Widget selectableWidget() {
    ...
    switch (sid) {
      ...
      case 2:
        return ListView.builder(
          itemCount: controller.filterCityArray.value.length,
          itemBuilder: (BuildContext context, int index) {
            return Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                checkBoxView(
                    controller.filterCityArray.value[index].text ?? "",
                    index,
                    controller.filterCityArray.value[index].isSelected ??
                        false),
                const Divider()
              ],
            );
      ...
      case 3:
        return ListView.builder(
          itemCount: controller.filterAreaArray.value!.length,
          itemBuilder: (BuildContext context, int index) {
            return Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                checkBoxView(
                    controller.filterAreaArray.value[index].text ?? "",
                    index,
                    controller.filterAreaArray.value[index].isSelected ??
                        false),
                const Divider()
              ],
            );
      ...
      case 4:
        return ListView.builder(
          itemCount: controller.filterGroupArray.value.length,
          itemBuilder: (BuildContext context, int index) {
            return Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                checkBoxView(
                    controller.filterGroupArray.value[index].text ?? "",
                    index,
                    controller.filterGroupArray.value[index].isSelected ??
                        false),
                const Divider()
              ],
            );
  ...
  void changeSelectionStatus(int index) {
    ...
    switch (sid) {
      ...
      case 2:
        controller.filterCityArray.value[index].isSelected =
            !(controller.filterCityArray.value[index].isSelected ?? false);
        controller.filterCityArray.refresh();
        break;
      case 3:
        controller.filterAreaArray.value[index].isSelected =
            !(controller.filterAreaArray.value[index].isSelected ?? false);
        controller.filterAreaArray.refresh();
        break;
      case 4:
        controller.filterGroupArray.value[index].isSelected =
            !(controller.filterGroupArray.value[index].isSelected ?? false);
        controller.filterGroupArray.refresh();
        break;
